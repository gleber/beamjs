...
exports(_VM) ->
    ?V8Obj([{"__doc__",
             "Operating System Specific Functions\n\nThe functions "
             "in this module are operating system specific. Careless "
             "use of these functions will result in programs that "
            ...
            {"cmd",
             erlv8_fun:new(fun cmd/2,
                           ?V8Obj([{"__doc__",
                                    "`cmd(name)` -> String\n\nExecutes Command in a command "
                                    "shell of the target OS, captures the standard output "
                                    "of the command and returns this result as a string."}]))},
            {"findExecutable",
             erlv8_fun:new(fun find_executable/2,
                           ?V8Obj([{"__doc__",
                                    "`findExecutable(name[, path])` -> String\n\nThese function "
                                    "look up an executable program given its name and a search "
                                    "path, in the same way as the underlying operating system.\n\n"
                                    "The absolute filename of the executable program Name "
                                    "is returned, or false if the program was not found."}]))},
            {"getEnv",
             erlv8_fun:new(fun getenv/2,
                           ?V8Obj([{"__doc__",
                                    "`getenv(varname)` -> String\n\nReturns the Value of "
                                    "the environment variable `varname`.\n\nReturns `false` "
                                    "if the environment variable is undefined."}]))},
            {"getFullEnv",
             erlv8_fun:new(fun getfullenv/2,
                           ?V8Obj([{"__doc__",
                                    "`getFullEnv()` -> Object\n\nReturns the object containng "
                                    "all environment variables."}]))},
            {"putEnv",
             erlv8_fun:new(fun putenv/2,
                           ?V8Obj([{"__doc__",
                                    "`putenv(varname, value)` -> true\n\nSets a new Value "
                                    "for the environment variable VarName."}]))},
            {"type",
             erlv8_fun:new(fun type/2,
                           ?V8Obj([{"__doc__",
                                    "`type()` -> [unix, linux]\n\nReturns the Osfamily and, "
                                    "in some cases, Osname of the current operating system."}]))},
            {"version",
             erlv8_fun:new(fun version/2,
                           ?V8Obj([{"__doc__",
                                    "<code>version()</code> -> [2,6,37] \n\nReturns the Osfamily "
                                    "and, in some cases, Osname of the current operating "
                                    "system."}]))}]).

cmd(#erlv8_fun_invocation{}, [Command]) when is_list(Command) ->
    os:cmd(Command).

find_executable(#erlv8_fun_invocation{}, [Command, Path])
  when is_list(Command) andalso is_list(Path) ->
...
type(#erlv8_fun_invocation{}, []) ->
    {Osfamily, Osname} = os:type(), ?V8Arr([Osfamily, Osname]).

version(#erlv8_fun_invocation{}, []) ->
    {Major, Minor, Release} = os:version(), ?V8Arr([Major, Minor, Release]).
