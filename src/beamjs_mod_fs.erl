...
-export([errno_desc/1, exports/1, init/1]).
            ...
            {"readdir",
             erlv8_fun:new(fun list_dir/2,
                           ?V8Obj([{"__doc__",
                                    ...
                                    "and '..'."}]))},
            {"rmdir",
             erlv8_fun:new(fun del_dir/2,
                           ?V8Obj([{"__doc__",
                                    "`rmdir(path, callback)`\n\nAsynchronous [rmdir(3)](http://man"
                                    "pages.ubuntu.com/manpages/maverick/en/man3/rmdir.3posix.html)"
                                    ".No arguments other than a possible exception are given "
                                    "to the completion callback."}]))}]).

read_file(#erlv8_fun_invocation{},
          [Filename, Encoding, #erlv8_fun{} = Callback])
  when is_list(Filename) andalso is_list(Encoding) ->
    case file:read_file(Filename) of
        {ok, B} when is_binary(B) -> Callback:call([null, binary_to_list(B)]);
        {error, Errno} ->
            Callback:call([{error, Filename ++ ": " ++ errno_desc(Errno)}, ""])
    end;
read_file(#erlv8_fun_invocation{} = I, [Filename, #erlv8_fun{} = Callback])
  when is_list(Filename) ->
    read_file(I, [Filename, "utf-8", Callback]);
read_file(#erlv8_fun_invocation{}, [Filename, Encoding])
  when is_list(Filename) andalso is_list(Encoding) ->
    {throw, {error, "No callback specified"}}.

list_dir(#erlv8_fun_invocation{}, [Path, #erlv8_fun{} = Callback])
  when is_list(Path) ->
    case file:list_dir(Path) of
        {ok, List} -> Callback:call([null, ?V8Arr(List)]);
        {error, Errno} ->
            Callback:call([{error, Path ++ ": " ++ errno_desc(Errno)}, ""])
    end.

del_dir(#erlv8_fun_invocation{}, [Path, #erlv8_fun{} = Callback])
  when is_list(Path) ->
    case file:del_dir(Path) of
        ok -> Callback:call([null]);
        {error, Errno} ->
            Callback:call([{error, Path ++ ": " ++ errno_desc(Errno)}, ""])
    end.
