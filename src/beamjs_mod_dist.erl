...
exports(VM) ->
    ...
    ?V8Obj([{"node", fun node/2}, {"Node", Node},
            {"ping",
             erlv8_fun:new(fun ping/2,
                           ?V8Obj([{"__doc__",
                                    "<code>ping(nodename)</code>\n\nTries to set up a connection "
                                    "to *nodename*.\n\nReturns `false` if it fails, or `true` "
                                    "if it is successful."}]))},
            {"localhost",
             erlv8_fun:new(fun localhost/2,
                           ?V8Obj([{"__doc__",
                                    "<code>localhost()</code>\n\nReturns the name of the "
                                    "localhost. If *beamjs* was started with the -name command "
                                    "line flag, it will return a fully qualified name."}]))},
            {"nodes", fun nodes/2}]).

prototype_Node() -> ?V8Obj([{"ping", fun ping/2}]).

node(#erlv8_fun_invocation{}, []) -> node().
...
localhost(#erlv8_fun_invocation{}, []) -> net_adm:localhost().

nodes(#erlv8_fun_invocation{vm = VM}, []) ->
    NodeCtor = erlv8_vm:retr(VM, {?MODULE, ctor_Node}),
    ?V8Arr([nodes_1(V1, NodeCtor) || V1 <- nodes()]).

nodes_1(Node, NodeCtor) ->
