...
prototype_Manager() ->
    ?V8Obj([{"addHandler", fun add_handler/2},
            {"deleteHandler", fun delete_handler/2}, {"swapHandler", fun swap_handler/2},
            {"whichHandlers", fun which_handlers/2}, {"notify", fun notify/2},
            {"syncNotify", fun sync_notify/2}]).

prototype_Handler() ->
    ?V8Obj([{"onEvent", handler_setter_fun("_onEvent")},
...
exports(VM) ->
    ...
    ?V8Obj([{"Manager", Manager}, {"Handler", Handler}]).
...
delete_handler(#erlv8_fun_invocation{this = This},
               [#erlv8_object{} = Handler | Args]) ->
    Pid = (?PID), gen_event:delete_handler(Pid, {?MODULE, Handler}, Args).

swap_handler(#erlv8_fun_invocation{this = This},
             [#erlv8_object{} = Handler1, #erlv8_object{} = Handler2]) ->
    Pid = (?PID),
    gen_event:swap_handler(Pid, {{?MODULE, Handler1}, [swap_handler]},
                           {{?MODULE, Handler2}, {gen_event, Handler2}}).

which_handlers(#erlv8_fun_invocation{this = This}, []) ->
    Pid = (?PID),
    ?V8Arr([which_handlers_1(V1) || V1 <- gen_event:which_handlers(Pid)]).

which_handlers_1({_, Id}) -> Id.

notify(#erlv8_fun_invocation{this = This}, [Event]) ->
    Pid = (?PID), gen_event:notify(Pid, Event).

sync_notify(#erlv8_fun_invocation{this = This}, [Event]) ->
    Pid = (?PID), gen_event:sync_notify(Pid, Event).
...
terminate(Args, Handler) ->
    case Handler:get_value("_onTerminate") of
        #erlv8_fun{} = F -> Handler:call(F, Args);
        _ -> undefined
    end.
